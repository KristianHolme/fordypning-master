function [gridcases, names] = getRSCGridcases(gridTypes, targetResolutions)
    ...
    configs = struct();
    configs.r10k   = struct('nx', 140,  'nz', 75,  'qt', 1.54,   't', 1.25,   'sig', [2,2]);
    configs.r50k   = struct('nx', 500,  'nz', 100, 'qt', 0.65,  't', 0.52,  'sig', [2,2]);
    configs.r100k  = struct('nx', 840,  'nz', 120, 'qt', 0.45,  't', 0.36,  'sig', [2,2]);
    configs.r200k  = struct('nx', 1180, 'nz', 170, 'qt', NaN,  't', 0.25,  'sig', [3,2]);
    configs.r500k  = struct('nx', 1870, 'nz', 270, 'qt', NaN, 't', 0.16,  'sig', [3,2]);
    configs.r1m    = struct('nx', 2640, 'nz', 380, 'qt', NaN, 't', 0.112, 'sig', [3,3]);
