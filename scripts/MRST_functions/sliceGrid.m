...
for k = 1:npts
    ...
    % clf
    % plotCellData(G, (1:numel(unique(poly.cellIx)))', unique(poly.cellIx));hold on;
    % plot3(poly.segments(:,1), poly.segments(:,2),poly.segments(:,3), 'r--')
    if G.griddim == 2
    ...
    % sliceFaces
    removedFaces = []; %KH: add: needed if cutCells.ic isempty
...
if isempty(gix)%KH add: no cut has happened
    return
end
