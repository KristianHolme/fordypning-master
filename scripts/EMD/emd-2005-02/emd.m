function [x, fval, flag] = emd(F1, F2, W1, W2, Func, opts)
%
...
[ind_m, ind_n] = ndgrid(1:m, 1:n);
linear_indices = ind_n(:) + (ind_m(:) - 1) * n;

% Create sparse matrices A1 and A2
A1 = sparse(ind_m(:), linear_indices, 1, m, m * n);
A2 = sparse(ind_n(:), linear_indices, 1, n, m * n);
...
% linear programming
[x, fval, flag] = linprog(f, A, b, Aeq, beq, lb, [], opts);
fval = fval / sum(x);
