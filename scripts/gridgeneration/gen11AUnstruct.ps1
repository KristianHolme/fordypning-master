...
foreach ($refinement_factor in $refinement_factors) {
    ...
    $filename = ".\..\..\grid-files\spe11a_ref" + $safe_refinement_factor + "_alg" + $algorithm + ".m"

    # Add the -setnumber Mesh.Algorithm $algorithm to specify the mesh algorithm
    & gmsh -2 spe11a.geo -setnumber refinement_factor $refinement_factor -setnumber Mesh.Algorithm $algorithm -format 'm' -o $filename
}
