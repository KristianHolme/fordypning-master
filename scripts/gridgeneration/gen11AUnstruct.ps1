...
foreach ($refinement_factor in $refs) {
    ...
    $filename = "spe11a_ref" + $safe_refinement_factor + "_alg" + $alg + ".m"
    ...
    & gmsh -2 spe11a.geo -setnumber refinement_factor $refinement_factor -setnumber Mesh.Algorithm $alg -format 'm' -o $savepath
}
Set-Location $dir